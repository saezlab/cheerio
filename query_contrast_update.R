...
mouse_swim_prot <- read_csv("~/R-projects/Collaborations/cheerio/raw_data/proteome/Maus Swim/Limma_results_V2.csv")%>%
  mutate(contrast_id = ifelse(grepl("2d",sample), "Mm_swim_prot_2d", "Mm_swim_prot_2wk"))

mouse_tac_prot <- read_csv("raw_data/proteome/Maus TAC/Limma_results_V1.csv")%>%
  filter(comparison.label %in% c("TAC vs Sham_TAC"))%>%
  mutate(contrast_id = "Mm_tac_prot_2wk")

# select the  first and remove the peptide versioning
peptide_selection <- function(df){
  df$first_protein_id_v <- sapply(strsplit(df$protein_id, "\\|"), `[`, 1)
  df$first_protein_id  <- gsub("\\..*","", df$first_protein_id_v )
  df %>% select(first_protein_id, protein_id, logFC, AveExpr, pvalue.limma, fdr.limma, B, contrast_id)
  
}

mouse_swim_prot<- peptide_selection(mouse_swim_prot)
mouse_tac_prot<- peptide_selection(mouse_tac_prot)
mm.prot <- rbind(mouse_swim_prot, mouse_tac_prot)

mm.prot %>% write.csv("data/mm.prot.csv")

mm.prot %>% pivot_wider(id_cols=  c(protein_id),
                        names_from = contrast_id,
                        values_from = logFC)
human_prot <- read_csv("raw_data/proteome/Human Prosser Paper/Limma_results_V1.csv")

human_prot <- human_prot%>% filter(comparison.label %in% c("cHyp vs normal", 
                                                           "HCMrEF vs normal", 
                                                           "HCMpEF vs normal", 
                                                           "DCM vs normal"))%>%
  mutate(comparison.label = str_replace_all(comparison.label, "normal", "NF"),
         comparison.label = str_replace_all(comparison.label, " ", ""),
         contrast_id = paste0("Hs_bulk_prot_", comparison.label))

human_prot$first_gene <- sapply(strsplit(human_prot$Gene.names , "\\;"), `[`, 1)
human_prot <- human_prot %>% select(first_gene, contrast_id,  Gene.names, Protein.Ids, Protein.names, 
                      logFC, AveExpr, pvalue.limma, fdr.limma,  B)

human_prot_short<- human_prot %>% 
  rename(gene = first_gene,
         FDR = fdr.limma)%>%
  dplyr::select(contrast_id, gene, logFC, FDR)

mm.prot_short <- mm.prot%>% 
  rename(gene = first_protein_id,
         FDR = fdr.limma)%>%
dplyr::select(contrast_id, gene, logFC, FDR)
### combine

joint_contrast_df= rbind(contrast_df, reheat.r, sc.gex2, contrast_m, PE,
                         human_prot_short, mm.prot_short)
...
joint_contrast_df <-
  ...
  mutate(ranks1= sign(logFC)*-log10(FDR),
         ...
         gene = (gene), # this will be updated in species_translation.R
         sig= FDR<0.05, 
...
saveRDS(joint_contrast_df, "data/contrasts_query_df_untranslated.rds")
joint_contrast_df<-readRDS( "data/contrasts_query_df_untranslated.rds")
