...
joint_contrast_df<-readRDS(paste0(project_path,"data/contrasts_query_df_untranslated.rds" ))
...
rat_gene_ids <- joint_contrast_df %>% filter(grepl("Rn", contrast_id))%>% pull(gene) %>% unique()
...
mouse_protein_to_mouse_gene_l<- quantify_mappings(m_prot_to_m_gene, protein_ids, "ensembl_peptide_id", "external_gene_name")
mouse_to_human_l <- quantify_mappings(mouse_to_human,mouse_gene_ids, "external_gene_name",  "hsapiens_homolog_associated_gene_name")
rat_to_human_l <-quantify_mappings(rat_to_human,rat_gene_ids, "external_gene_name", "hsapiens_homolog_associated_gene_name" )

mouse_protein_to_mouse_gene_stats <- lapply(mouse_protein_to_mouse_gene_l, length)
mouse_to_human_stats  <- lapply(mouse_to_human_l, length)
rat_to_human_stats  <- lapply(rat_to_human_l, length)

# Combine the data into a single data frame
mapping_summary <- tibble(
  Conversion = c("Mouse Protein to Mouse Gene", "Mouse Gene to Human Gene", "Rat Gene to Human Gene"),
  one_to_none = c(mouse_protein_to_mouse_gene_stats$one_to_none, 
               mouse_to_human_stats$one_to_none, 
               rat_to_human_stats$one_to_none),
  one_to_many = c(mouse_protein_to_mouse_gene_stats$one_to_many, 
                 mouse_to_human_stats$one_to_many, 
                 rat_to_human_stats$one_to_many),
  many_to_one = c(mouse_protein_to_mouse_gene_stats$many_to_one, 
                 mouse_to_human_stats$many_to_one, 
                 rat_to_human_stats$many_to_one),
  one_to_one = c(mouse_protein_to_mouse_gene_stats$one_to_one, 
                 mouse_to_human_stats$one_to_one, 
                 rat_to_human_stats$one_to_one)
)

# Pivot the data frame for easier plotting
mapping_summary_long <- mapping_summary %>%
  pivot_longer(cols = c(one_to_none, one_to_many, many_to_one, one_to_one), names_to = "Status", values_to = "Count")%>%
  mutate(logCount = log10(Count), 
         logCount = ifelse(is.infinite(logCount), 0, logCount))%>%
  group_by(Conversion)%>%
...
mouse_prot_mapped<- joint_contrast_df%>% 
  ...
  left_join(m_prot_to_m_gene%>%
              filter(ensembl_peptide_id %in% mouse_protein_to_mouse_gene_l$one_to_one)%>%
...
rat_mapped= joint_contrast_df%>% 
  ...
  left_join(rat_to_human %>% 
              filter(external_gene_name %in% rat_to_human_l$one_to_one)%>% 
...
Animal_df <- rbind(rat_mapped,mouse_mapped, mouse_prot_mapped_to_gene )
...
saveRDS(joint_df_translated, "data/contrasts_query_df_translated.rds")
